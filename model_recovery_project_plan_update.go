/*
HyperOne

HyperOne API

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// RecoveryProjectPlanUpdate struct for RecoveryProjectPlanUpdate
type RecoveryProjectPlanUpdate struct {
	Name *string `json:"name,omitempty"`
}

// NewRecoveryProjectPlanUpdate instantiates a new RecoveryProjectPlanUpdate object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRecoveryProjectPlanUpdate() *RecoveryProjectPlanUpdate {
	this := RecoveryProjectPlanUpdate{}
	return &this
}

// NewRecoveryProjectPlanUpdateWithDefaults instantiates a new RecoveryProjectPlanUpdate object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRecoveryProjectPlanUpdateWithDefaults() *RecoveryProjectPlanUpdate {
	this := RecoveryProjectPlanUpdate{}
	return &this
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *RecoveryProjectPlanUpdate) GetName() string {
	if o == nil || o.Name == nil {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RecoveryProjectPlanUpdate) GetNameOk() (*string, bool) {
	if o == nil || o.Name == nil {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *RecoveryProjectPlanUpdate) HasName() bool {
	if o != nil && o.Name != nil {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *RecoveryProjectPlanUpdate) SetName(v string) {
	o.Name = &v
}

func (o RecoveryProjectPlanUpdate) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Name != nil {
		toSerialize["name"] = o.Name
	}
	return json.Marshal(toSerialize)
}

type NullableRecoveryProjectPlanUpdate struct {
	value *RecoveryProjectPlanUpdate
	isSet bool
}

func (v NullableRecoveryProjectPlanUpdate) Get() *RecoveryProjectPlanUpdate {
	return v.value
}

func (v *NullableRecoveryProjectPlanUpdate) Set(val *RecoveryProjectPlanUpdate) {
	v.value = val
	v.isSet = true
}

func (v NullableRecoveryProjectPlanUpdate) IsSet() bool {
	return v.isSet
}

func (v *NullableRecoveryProjectPlanUpdate) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRecoveryProjectPlanUpdate(val *RecoveryProjectPlanUpdate) *NullableRecoveryProjectPlanUpdate {
	return &NullableRecoveryProjectPlanUpdate{value: val, isSet: true}
}

func (v NullableRecoveryProjectPlanUpdate) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRecoveryProjectPlanUpdate) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


